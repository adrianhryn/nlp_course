    ...
    for sent in sents:
        ...
        for word in sent:
            if len(word) > max_word_length:
                # just adds first 21 characters
                words_padded.append(word[:max_word_length])
            else:
                #adds all existing characters and char_pad_token*necessary times
                words_padded.append(word + [char_pad_token] *
                                    (max_word_length - len(word)))
        # adds all padded words and word consists of all pad tokens multiply necessery amount of time
