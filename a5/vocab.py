...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        return [[[self.char2id[i] for i in "".join(["{", w, "}"]).replace("", " ").strip().split(" ")] for w in s] for s
                in sents]
        ### END YOUR CODE
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        char_indeces = self.words2charindices(sents)
        padded_char_indeces = pad_sents_char(char_indeces, self.char2id['<pad>'])

        tensor = torch.tensor(padded_char_indeces, dtype=torch.long, device=device)
