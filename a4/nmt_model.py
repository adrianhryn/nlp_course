...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.encoder = nn.LSTM(embed_size, hidden_size=hidden_size, dropout=dropout_rate, bidirectional=True)
        self.decoder = nn.LSTMCell(embed_size + hidden_size, hidden_size=hidden_size)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        decoder_init_hidden = self.h_projection(last_hidden)
        last_cell = torch.cat((last_cell[0, :], last_cell[1, :]), 1)
        decoder_init_cell = self.c_projection(last_cell)
        dec_init_state = (decoder_init_hidden, decoder_init_cell)
        ### END YOUR CODE
    ...
    def beam_search(self, src_sent: List[str], beam_size: int=5, max_decoding_time_step: int=70) -> List[Hypothesis]:
        ...
        @param src_sent (List[str]): a single source sentensh run.sh vocabce (words)
